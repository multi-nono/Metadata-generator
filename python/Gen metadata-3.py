...
class App:
    ...
    def _update_command_display(self):
        ...
        # Ajout de la description principale
        if self.description_content:
            metadata_string += f"\\u0003description\\u0002{self.description_content}\\u0003"
        
        # Ajout de la description courte (s'insère après la description si elle existe, sinon elle commence la metadata)
        if self.short_description_content:
             # Si il y a déjà une description principale, le short_description s'ajoute à la suite
    ...
    def _open_text_editor(self, title, initial_text, set_content_callback):
        ...
        def _apply_formatting(event=None):
            ...
            # On cherche \u001b(c@# suivi de 6 caractères hexadécimaux et une parenthèse fermante
            ...
            # On itère sur les correspondances pour appliquer la couleur au texte qui suit le code
        ...
        def confirm_text():
            ...
            content = text_editor.get(1.0, END).replace("\n", "\\n").strip()
            set_content_callback(content)
        ...
        # Gérer l'insertion automatique de \n et le formatage en direct
        # Bind pour la touche Entrée: insère \n et empêche le comportement par défaut
        text_editor.bind("<Return>", lambda event: (text_editor.insert(INSERT, "\\n"), "break"))
