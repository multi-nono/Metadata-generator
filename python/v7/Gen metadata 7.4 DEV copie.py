...
class App:
    ...
    def _update_command_display(self):
        ...
        standard_metadata_content_parts = [] # Stores "key\u0002value" strings
        ...
        if self.tool_capabilities_data is not None:
            ...
            standard_metadata_content_parts.append(f"tool_capabilities\\u0002{tool_caps_json_escaped}")

        # 2. Description
        if self.description_content:
            standard_metadata_content_parts.append(f"description\\u0002{self.description_content}")

        # 3. Short Description
        if self.short_description_content:
            standard_metadata_content_parts.append(f"short_description\\u0002{self.short_description_content}")

        # Build the standard metadata string with its prefix and internal \u0003
        final_metadata_string_core = ""
        if standard_metadata_content_parts:
            ...
            final_metadata_string_core += "\\u0003" + "\\u0003".join(standard_metadata_content_parts)

            # Only add the final \u0003 for the standard block if item color is NOT present
            # because item color's block starts with \u001bE\u0003 which acts as the next separator.
            if not self.item_color_hex:
                final_metadata_string_core += "\\u0003"
