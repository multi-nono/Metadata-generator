...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Metadata items generator (By multi_nono / Nono)")
    ...
    def _create_widgets(self):
        item_frame = tk.LabelFrame(self.root, text="Informations of the item", padx=10, pady=10)
        item_frame.pack(pady=10, padx=10, fill="x")

        tk.Label(item_frame, text="item's name (ex: default:stone):").grid(row=0, column=0, sticky="w", padx=5, pady=2)
        ...
        btn_desc = tk.Button(metadata_buttons_frame, text="modify the description", command=self._open_description_editor)
        ...
        command_frame = tk.LabelFrame(self.root, text="the final /give command :", padx=10, pady=10)
        ...
        copy_button = tk.Button(self.root, text="Copy the command", command=self._copy_command)
    ...
    def _open_description_editor(self):
        self._open_text_editor("modify the description of the item (in the inventory)", self.description_content, self._set_description_content)

    def _open_short_description_editor(self):
        self._open_text_editor("modify the short description of the item (when you take it in hand)", self.short_description_content, self._set_short_description_content)
    ...
    def _open_text_editor(self, title, initial_text, set_content_callback):
        ...
        def toggle_code_visibility():
            ...
            if editor_window.show_codes:
                toggle_button.config(text="hide color code")
            else:
                toggle_button.config(text="show color code")
            _apply_formatting()
        ...
        toggle_button = tk.Button(button_frame, text="hide color code" if editor_window.show_codes else "show color code", command=toggle_code_visibility)
    ...
    def _open_item_color_picker(self):
        ...
        color_window.title("color of the item")
        ...
        tk.Label(color_window, text="choosen color :").pack(pady=5)
        ...
        btn_cancel = tk.Button(color_window, text="cancel the color", command=cancel_color)
    ...
    def _copy_command(self):
        ...
        messagebox.showinfo("Copied !", "the command has been copied in the clipboard")
