...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("générateur de metadata DEV debug")
    ...
    def _update_command_display(self):
        ...
        all_metadata_blocks = [] # List to hold all complete metadata blocks

        # 1. Tool Capabilities Block (includes x_enchanting prefix and its own \u0003 suffix)
        ...
        # Combine all blocks. Each block starts with \u0003 (or \u0001x_enchanting\u0002\u0003 for tool_caps)
        # We will add the final \u0003 at the very end if needed.
        final_metadata_string_core = "".join(all_metadata_blocks)

        # 4. Item Color Block (special handling, always appended at the end, includes its own \u0003)
        item_color_block = ""
        if self.item_color_hex:
            item_color_block = f"\\u001bE\\u0003color\\u0002{self.item_color_hex}\\u0003"
        
        # Determine the final \u0003 for the entire metadata string
        # It's needed if there are core metadata blocks AND the item color block is NOT present (as item color has its own \u0003)
        final_terminator_u0003 = ""
        if final_metadata_string_core and not item_color_block:
            final_terminator_u0003 = "\\u0003"

        # Combine core metadata, the final terminator, and item color
