...
class App:
    ...
    def _update_command_display(self):
        ...
        metadata_blocks_for_join = [] # List to hold parts that will be joined by \u0003
        item_color_block = ""
        ...
        if self.tool_capabilities_data is not None:
            ...
            metadata_blocks_for_join.append(f"tool_capabilities\\u0002{tool_caps_json_escaped}")

        # 2. Description
        if self.description_content:
            metadata_blocks_for_join.append(f"description\\u0002{self.description_content}")

        # 3. Short Description
        if self.short_description_content:
            metadata_blocks_for_join.append(f"short_description\\u0002{self.short_description_content}")

        # Construct the main metadata string with \u0003 separators and the x_enchanting prefix
        final_metadata_string = ""
        if metadata_blocks_for_join:
            # Add the global prefix for x_enchanting if any standard metadata block is present
            final_metadata_string += "\\u0001x_enchanting\\u0002"
            # Join the blocks with \u0003 and add a final \u0003
            final_metadata_string += "\\u0003" + "\\u0003".join(metadata_blocks_for_join) + "\\u0003"
        ...
        # Append the item color block at the very end
        final_metadata_string += item_color_block
