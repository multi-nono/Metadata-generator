...
class App:
    ...
    def _open_text_editor(self, title, initial_text, set_content_callback):
        ...
        # État pour masquer/afficher les codes
        editor_window.show_codes = True # Initialisé à True (codes visibles)
        ...
        def _apply_formatting(event=None):
            ...
            for tag in text_editor.tag_names():
                # Supprimer les tags de couleur et les tags de visibilité des codes
                if tag.startswith("color_") or tag == "hidden_code_tag" or tag == "visible_code_tag" or tag == "default_color":
                    text_editor.tag_delete(tag)
            
            content_in_editor = text_editor.get(1.0, END) # Récupérer le contenu actuel
            color_pattern = r"(\\u001b\(c@#([0-9a-fA-F]{6})\))"
            
            last_processed_index = "1.0"
            current_color = "black" # Couleur par défaut pour le texte non formaté
            ...
            for match in re.finditer(color_pattern, content_in_editor):
                ...
                # Appliquer la couleur courante au segment de texte *avant* ce code
                ...
                # Gérer le code de couleur lui-même
                if not editor_window.show_codes: # Si les codes doivent être masqués
                    text_editor.tag_add("hidden_code_tag", code_start_index, code_end_index)
                    text_editor.tag_config("hidden_code_tag", foreground=text_editor.cget("bg"), background=text_editor.cget("bg"), font=("Arial", 1)) # Rendre invisible et très petit
                else: # Les codes sont affichés
                    text_editor.tag_add("visible_code_tag", code_start_index, code_end_index)
                    text_editor.tag_config("visible_code_tag", foreground="black", background=text_editor.cget("bg"), font=("Arial", 10)) # Rétablir la taille normale et la couleur noire

                # La couleur pour le texte *après* ce code
                current_color = hex_color
                last_processed_index = code_end_index # Le prochain segment commence après la fin du code

            # Appliquer la dernière couleur active au texte restant après le dernier code
            ...
            try:
                ...
                text_editor.see(INSERT) # Faire défiler pour que le curseur soit visible
            except tk.TclError:
                # Si le curseur n'est plus valide (ex: le texte a été supprimé sous le curseur)
        ...
        def confirm_text():
            # Le contenu du Text widget contient toujours les vrais codes \u001b(...),
            # car les '|' ne sont que des widgets superposés pour l'affichage.
            raw_content = text_editor.get(1.0, "end-1c") # Exclut le dernier \n implicite de Text widget
        ...
        def toggle_code_visibility():
            ...
            _apply_formatting() # Réappliquer le formatage avec le nouvel état
        ...
        text_editor.delete(1.0, END) # S'assurer que le widget est vide avant d'insérer
        text_editor.insert(END, display_text) # Insérer le vrai texte avec les codes \u001b(...)
        _apply_formatting() # Appliquer le formatage
    ...
    def _open_item_color_picker(self):
        ...
        color_window.geometry("300x200") # Increased height to accommodate new button
        ...
        def cancel_color():
            self.item_color_hex = "" # enlever la couleur de l'item
            color_window.destroy()
            color_window.grab_release()
            self._update_command_display()
